...
def check_deadbeef_version():
	...
	decode_version_out = str(read_version_out, encoding='utf8')
	if __module_deadbeef_version__ in decode_version_out:
		pass
	else:
...
def is_track_loaded():
	...
	decode_loadedornot_out = str(loadedornot_out, encoding='utf8')
	if "nothing" in decode_loadedornot_out:
		print("Track loaded!")
	else:
...
def deadbeef_current_track(word, word_eol, userdata):
	...
	announce_track = nickname + str(decode_read_track_out).strip('\n')
...
def deadbeef_pause_track(word, word_eol, userdata):
	...
	decode_track_playing_time_out = str(track_playing_time_out, encoding='utf8')
	print("Current track paused! %s" % decode_track_playing_time_out)
